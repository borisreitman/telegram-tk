...
Examples:
  echo 123456 | .venv/bin/python tg_delete_users.py @mychannel --dry-run
  .venv/bin/python tg_delete_users.py @mychannel --yes --file ids.txt
  grep -i spam users.tsv | cut -f1 | .venv/bin/python tg_delete_users.py @mychannel --yes
"""
...
async def run(channel: str, user_ids: list[int], dry_run: bool, yes: bool) -> None:
    ...
    if not await client.is_user_authorized():
        raise SystemExit("Not authorized. Run: .venv/bin/python tg_auth.py")
